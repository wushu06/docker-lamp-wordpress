...
foreach ($array_of_groupid_name_pair as $single_groupid_name_pair) {
    $html .= "<option value=" . $single_groupid_name_pair->group_id . ' >' . esc_html($single_groupid_name_pair->name) . "</option>";
}
$html .= '</select>';
...
$cnt = count($array_of_groupname_price_pair);
...
if (count($array_of_groupname_price_pair) > 0 && false != $array_of_groupname_price_pair) {
    ...
    <script type="text/javascript">
        jQuery( function () {  //Print all combinations saved in the database except first combination.
            ...
            wdm_temp_select_holder.find( 'option[value="<?php echo $array_of_groupname_price_pair[ $j ]->group_id; ?>"]' ).attr( 'selected', true );
            ...
            qty_textbox = "<td class='wdm_left_td'><input type='number' min = '1' name='wdm_woo_group_qty[]' class='wdm_qty' value='<?php echo $array_of_groupname_price_pair[ $j ]->min_qty; ?>' /></td>";
<?php
if ($array_of_groupname_price_pair[ $j ]->price_type == 2) {
    //Show Price's Textbox
    ?>
    price_textbox = "<td colspan=3 class='wdm_left_td'><input type='text' name='wdm_woo_group_price[]' class='wdm_price csp-percent-discount' value='<?php echo wc_format_localized_price($array_of_groupname_price_pair[ $j ]->price); ?>' /></td><td><a class='wdm_remove_pair_link' href='#' id='group_remScnt'  >";
    <?php
} else {
    //Show Price's Textbox
    ?>
    price_textbox = "<td colspan=3 class='wdm_left_td'><input type='text' name='wdm_woo_group_price[]' class='wdm_price' value='<?php echo wc_format_localized_price($array_of_groupname_price_pair[ $j ]->price); ?>' /></td><td><a class='wdm_remove_pair_link' href='#' id='group_remScnt'  >";
    <?php
}
?>
            //Show Price's Textbox
            ...
            remove_row_button = "<img alt='Remove Pair' title='Remove Pair' class='remove_group_price_pair_row_image' src='<?php echo $remove_image_path; ?>'/></a>";

            add_new_row = '';
    <?php if ($j == count($array_of_groupname_price_pair) - 1) { ?>
                //Add new pair button
                add_new_row = "<a class='wdm_add_group_pair_link' href='#' id='wdm_add_new_group_price_pair'><img class='add_new_row_image' src='<?php echo $add_image_path ?>' /></a>";
    <?php
} ?>
            //end row
            end_row = "</td></tr>";
...
$array_of_values_to_be_passed_to_js = array(
    ...
    'add_new_pair'           => __('Add New Pair', CSP_TD),
    'add_new_group_text'  => __('Add New Group-Price Pair', CSP_TD),
);
