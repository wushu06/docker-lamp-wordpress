...
$output .= "\t\t\t\t" . '<label for="wpsl-search-input">' . esc_html( $wpsl->i18n->get_translation( 'search_label', __( 'Your location', 'wpsl' ) ) ) . '</label><br>' . "\r\n";
...
if ( $wpsl_settings['radius_dropdown'] || $wpsl_settings['results_dropdown']  ) {
    ...
    if ( $wpsl_settings['radius_dropdown'] ) {
        ...
        $output .= "\t\t\t\t\t" . '<label for="wpsl-radius-dropdown">' . esc_html( $wpsl->i18n->get_translation( 'radius_label', __( 'Search radius', 'wpsl' ) ) ) . '</label><br>' . "\r\n";
    ...
    if ( $wpsl_settings['results_dropdown'] ) {
        ...
        $output .= "\t\t\t\t\t" . '<label for="wpsl-results-dropdown">' . esc_html( $wpsl->i18n->get_translation( 'results_label', __( 'Results', 'wpsl' ) ) ) . '</label><br>' . "\r\n";
...
$output .= "\t\t\t\t" . '<div class="wpsl-search-btn-wrap"><input id="wpsl-search-btn" type="submit" value="FIND A DISTRIBUTOR"></div>' .	"\r\n";
